...
**A web page is not the door.** The door's code answers JSON or an empty body, never a page (all of `doPost` sits in one `try` that answers `server`). A page in its place (`bad_response`) comes from Google, in front of the door. After this door has answered properly within the hour, the phone calls it a hiccup, says so, and tries again in 20 s. Before that — or after saving a different URL — the URL is wrong. The phone keeps a few plain words of the page (no link, host or token) to show on STUCK. The one such reply ever caught on a live phone was `walker-door` itself — the door's answer to a plain GET. A POST only becomes a GET by following a redirect, so something on Google's side redirected the request back to `/exec` on the way: a knock, not the request. The phone names it that way, keeps what it was sending, and retries under the same id — the door's receipts make the retry safe whether or not `doPost` had already run.
