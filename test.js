...
if(!SRC.includes('text/plain;charset=utf-8'))secretHits.push('door fetch missing text/plain Content-Type');
const doorFn=SRC.slice(SRC.indexOf('async function landRawViaDoor'),SRC.indexOf('async function landRawViaDrive'));
if(/Content-Type":"application\/json"/.test(doorFn))secretHits.push('door fetch still uses application/json');
...
dcheck('keyed unreachable toast',/Door unreachable/.test(D.dumpHoldToast({code:'no_door'})));
D.setDoorUrl('');
